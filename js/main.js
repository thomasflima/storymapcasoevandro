(function ($) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (typeof (slider) !== 'undefined' && slider !== null) {
                    ...
                    var SwiperNew = new Swiper(swiperObj, sliderOptions);
                } else {
                    // Apply isotope
                    if (!_this.find('.wow').length > 0) {
                        _this.find('.grid-item').css('visibility', 'hidden');
    ...
    /****** Open pictures modal ******/
    $(document).on("click", ".nome-persona", function (e) {
        let nome = $(this).data("nome"),
            descricao = $(this).data("descricao"),
            img = $(this).data("src"),
            posicao = $(this).data("posicao");
        $("#imagem-pessoa").attr("src", img).attr("alt", nome);
        $("#posicao-pessoa").html(posicao);
        $("#nome-pessoa").html(nome);
        $("#descricao-pessoa").html(descricao);
        $('#modalPictures').modal('show')
    });

    $(document).on("click", "#closeModalPictures", function (e) {
        $('#modalPictures').modal('hide')
    });

    /****** Close Modal Anos ******/
    $(document).on("click", ".closeModal16Anos", function (e) {
        $('#modal16Anos').modal('hide')
    });
    ...
    $(document).on("click", ".change-map", function (e) {
        ...
        let map = $(this).data("src");
        $("#mapaOcorrencias").attr("src", map);
    });

    /****** Alterar mapa 2  ******/
    $(document).on("click", ".change-map-year", function (e) {
        $(this).parent().find(".active").removeClass("active")
        $(this).addClass("active")
        let map = $(this).data("src");
        $("#yearMap").attr("src", map);
    });

    /****** Teste Toggle ******/
    $(document).on('click', '.teste-fs', function (e) {
    ...
    $(window).on('load', function () {
        ...
        /****** start slick ******/
        setSlick($(".carrousel-1"));

        /****** start slick2 ******/
        setSlick2($(".carrousel-2"));

        /****** Modal Idade ******/
        $('#modal16Anos').modal('show')
    ...
    function setSlick(div) {
        ...
        if (div.length > 0) {
            div.slick({
                ...
                autoplaySpeed: 10000,
                adaptiveHeight: true,
                prevArrow: '<i class="fa-4x cursor-pointer red fas fa-angle-left position-absolute left-m50px"></i>',
                nextArrow: '<i class="fa-4x cursor-pointer red fas fa-angle-right position-absolute right-m50px"></i>',
            });
        }
    }

    /***** SLICK CARROUSEL 2 *****/
    function setSlick2(div) {
        console.log(div)
        if (div.length > 0) {
            div.slick({
                dots: false,
                ...
                autoplaySpeed: 10000,
                adaptiveHeight: true,
                prevArrow: '<i class="fa-4x cursor-pointer text-white fas fa-angle-left position-absolute left-m50px"></i>',
                nextArrow: '<i class="fa-4x cursor-pointer text-white fas fa-angle-right position-absolute right-m50px"></i>',
            });
        }
    }

    /****** GET FULLSCREEN ******/
    function getFullScreen(div) {
